...
router.get('/:naziv', (req, res) => {
    ...
    if (!pronadeni_brod) {
        return res.status(404).json({greska: `Brod s nazivom ${req.params.naziv} ne postoji u listi`});
    }
...
router.post('/', (req, res) => {
    ...
    const hasExactKeys = 
        ...
        bodyKeys.every((k, i) => k === requiredKeys[i]);


    if (!hasExactKeys) {
        return res.status(400).json({greska: `Tijelo zahtjeva mora sadrzavati tocne kljuceve: ${REQUIRED_KEYS.join(', ')}`})
    }
