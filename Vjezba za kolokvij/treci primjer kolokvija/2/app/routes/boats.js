...
const boatsSeed = JSON.parse(fs.readFileSync(boatsFile, 'utf-8'));
...
router.get('/:naziv', (req, res) => {
    const naziv = normalizeName(req.params.naziv);
    const pronadeni_brod = boats.find(b => normalizeName(b.naziv) === naziv);

    if (!pronadeni_brod) {
        return res.status(404).json({greska: `Brod s nazivom ${req.params.naziv} nije pronaden.`});
    }

    res.status(200).json(pronadeni_brod);
})

router.post('/', (req, res) => {
    const REQUIRED_KEYS = ['naziv', 'tip', 'duljina', 'cijenaPoDanu', 'motor_hp'];
    const bodyKeys = Object.keys(req.body).sort();
    const required_keys = [...REQUIRED_KEYS].sort();

    const hasExactKeys = bodyKeys.length === required_keys.length && bodyKeys.every((k, i) => k === required_keys[i]);

    if (!hasExactKeys) {
        return res.status(400).json({greska: `Tijelo zahtjeva mora sadrzavati tocno ove parametre: ${REQUIRED_KEYS.join(', ')}`})
    }
