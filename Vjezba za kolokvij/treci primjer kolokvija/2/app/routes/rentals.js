...
router.get('/', (req, res) => {
    if (rentals.length < 1) {
        return res.status(404).json({ greska: "Lista najmova je prazna" });
    }
...
router.post('/', (req, res) => {
    ...
    if (!boat) {
        return res.status(404).json({ greska: `Nije pronaden brod sa ID-om ${boatId}` });
    }
    if (typeof boat.cijenaPoDanu !== 'number') {
        return res.status(500).json({ greska: 'Brod nema definirani cijenu' });
    }
